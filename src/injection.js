...
async function main() {
    ...
    let [challenge_js, interception_js, vendor_js, bundle_js, bundle_css, twitter_text, x_theme_css] =
        await Promise.allSettled([
            ...
            fetch(chrome.runtime.getURL("/files/x-theme.css")).then(r => r.text()),
        ]);
    ...
    let x_theme_style = document.createElement("style");
    x_theme_style.innerHTML = x_theme_css.value;
    document.head.appendChild(x_theme_style);
    ...
    // Add X theme override CSS after bundle CSS
    let x_override_css = await fetch(chrome.runtime.getURL("/files/x-override.css")).then(r => r.text());
    let x_override_style = document.createElement("style");
    x_override_style.innerHTML = x_override_css;
    document.head.appendChild(x_override_style);
